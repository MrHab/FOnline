...
— обычная локация 320 × 320 м. Её собирает конструктор из кусков по 40 м
(`src/server/zone-builder.js`, куски — `data/zones/chunks/`), детерминированно
по зерну зоны; клиент строит её на лету из набора префабов без своей сцены.
Зону можно закрепить для ручной правки (`node tools/freeze-zone.js z_CC_RR` →
`data/zones/authored/<id>.json`, `--release` возвращает генератору). Зоны
