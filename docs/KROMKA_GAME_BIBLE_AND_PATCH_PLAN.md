...
| потребление и резервы поселений, население, миграция, беженцы, аварийные поставки (`settlementLife`) | выключено |
| добыча и производство поселений, закупочные и сырьевые задания (`npcProduction`) | выключено |
| караваны и их сопровождение (`worldCaravans`) | выключено |
| прежние враждебные отряды на карте (`hostileWorldParties`, `visibleWorldParties`) | выключено, заменены A-Life |
...
восстановление», миграция и беженцы, производство и караваны — выключена
флагами `settlementLife`, `npcProduction` и `worldCaravans`. Поселения v3 —
хабы услуг без потребления и производства. Правила выключенной модели описаны в
...
игроков (транспорт — отдельная будущая работа). Флаг
`worldModel.worldCaravans` выключает постоянные, экспортные и торговые караваны
вместе с заданиями сопровождения.

Видимых отрядов прежней живой пустоши на карте нет (`visibleWorldParties` и
`hostileWorldParties` выключены). Рейдеры и мутанты живут группами A-Life в
опасных клетках (раздел 18.1): приходят в сцены с края, гибнут насовсем и видны
на карте в радиусе, который даёт «Странник».
...
угрозе, ремонтные бригады при восстановлении — относятся к выключенному
`settlementLife`.
...
бросает шанс стычки — жёлтая 4%, красная 10%, чёрная 18%; навык «Странник»
снижает его вдвое на максимуме; окрестности начала и конца пути (3 км) не
...
- Враждебных отрядов прежней живой пустоши нет (`worldModel.hostileWorldParties
  = false`): налётчики и звери живут в клетках. Встречи из
...
**Видимость на карте и клетки Сердцевины.** «Странник»
отвечает не только за скрытность, но и за то, что игрок видит на глобальной
карте. Группы A-Life и других игроков видно в радиусе 4 км + 10 км × навык
(навык 20% даёт 4 км, 100% — 14 км); другого игрока — ещё и с поправкой на
его «Странника»: он виден лишь на доле радиуса смотрящего 1 − 0,5 × навык
(при 100% — на половине). Сервер пересчитывает это раз в 2 с и присылает
...
карте по «Страннику», марки на счёте аккаунта и переключаемый фокус. Остаётся
