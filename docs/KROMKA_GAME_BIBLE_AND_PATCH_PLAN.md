...
### 14.5. Ресурсные цепочки и рынок

Основная хозяйственная цепочка повторяет географию Кромки:

| Источник | Перевозимый груз | Получатель | Результат |
|---|---|---|---|
| Северные затворы и Меловая низь | вода и использованная синь | поселения и Р-12 | питьевая вода и материал для регенерации |
| Рудная дуга | руда, лом, керамика и детали | ремонтные и производственные узлы | исправная инфраструктура и готовые изделия |
| Срединная жила | пища и лекарственное сырьё | города, гарнизоны и караванные узлы | здоровье населения и снабжение отрядов |
| Стеколье | электроника, измерители и нестабильные образцы | Контур и сложные мастерские | детекторы, автоматика и прогноз Сдвигов |
| Нулевая котловина | восстановленная синь и химические реагенты | весь цикл «Каскада» | очистка воды, энергия и высокоуровневое производство |

Производство требует сырья, рабочей силы, исправного оборудования и времени.
Низкая безопасность уменьшает фактическую производительность, а активный бой
останавливает погрузку. Груз не телепортируется: товар покидает склад источника
при формировании каравана и зачисляется получателю ровно один раз после
прибытия. Ограбление, спасение и частичная потеря груза сохраняются в общем
состоянии.

Цены являются локальными. Избыток может снизить базовую цену до 85%, нехватка
поднять её до 135%, а подтверждённый кризис — максимум до 160%. Эти пределы не
дают рынку стать бесполезным или бесконечным источником марок. Торговец
объясняет изменение короткой причиной: «дорога закрыта», «караван пропал»,
«склад переполнен» или «Управа снова победила воду отчётом».
...
- караван сравнивает груз, спрос, длину пути, отношения и известную опасность;
- патруль защищает атакуемую точку, усиливает слабый участок или перехватывает
  обнаруженную угрозу;
- рейдеры предпочитают богатые караваны и слабые склады, но избегают явно более
  сильного противника;
- мутанты охотятся и кормятся вокруг логова, реагируют на шум, Сдвиги и
  уменьшение доступной добычи;
- отряд поддержки доставляет аварийный груз в кризисную точку;
- беженцы выбирают ближайшее доступное поселение с водой и безопасностью.

Решение фиксирует цель и короткий код причины. Повторный выбор имеет задержку,
чтобы отряды не дёргались между равными целями. При встрече несовместимых групп
создаётся видимая мировая активность. Игрок может вмешаться, но не командует
отрядом и не присоединяет его к себе.

Если игроков рядом нет, столкновение разрешается сервером по силе, состоянию,
местности и снабжению. Автоматический результат не создаёт личный лут: он меняет
численность отрядов, груз, безопасность точки и последующие контракты. При входе
игрока сервер материализует оставшихся участников и текущее состояние боя, а не
создаёт встречу заново.
...
каждую смену владельца выходит один NPC-гарнизон с платформы. Смерть в
Сердцевине — частичная потеря: выпадает рюкзак, экипировка и установленные
стабилизированные артефакты сохраняются. См. `docs/WORLD_ZONES_ARTIFACTS_2026-09.md`.
...
| Режим | Атака игроков | Потеря при смерти |
|---|---|---|
| Мирный | запрещена | нет |
| PvP | разрешена | расходники и часть переносимого груза |
| Полный лут | разрешена | переносимый инвентарь и патроны в магазинах |
...
требования, время и результат. Полевой крафт и офлайн-работа базы — разные
очереди и не дублируют один заказ.

Ремонт расходует комплект и материалы. Разбор атомарно уничтожает исходный
...
Основная валюта — марки Тракта. Торговать можно с NPC, автоматами и другим
игроком по двустороннему подтверждению. До сделки показываются покупка,
продажа, итог, новый вес, вместимость и влияние «Бартера».

Запас NPC ограничен состоянием поселения. Цена учитывает базовую стоимость,
навык, репутацию и дефицит живой пустоши, оставаясь в диапазоне 85–160%. Синь,
редкие артефакты и лучшие довоенные детали не возникают из бесконечного
магазина.
...
|---|---|
| Механик | стоимость ремонта −20%, улучшения генератора и транспорта |
| Оружейник | износ созданного оружия −20%, особые оружейные модули |
| Химик | выход лекарств и фильтров +25%, обработка токсичных материалов |
| Врач | восстановление тяжёлых травм на 50% быстрее, отдых даёт временные +10 HP |
| Исследователь Искажений | штрафы стабилизированных артефактов −20%, работа с редкими сростками |
| Радист | предупреждение о Сдвиге на 3 минуты раньше, отметка ближайшего убежища |
| Разведчик | две ресурсные отметки или безопасный маршрут после каждого Сдвига |
| Торговец | цены обычных товаров лучше на 8%, дополнительные заказы |
| Кладовщик | объём хранилища +40%, производительность станков +15% |
| Начальник охраны | защита базы от сюжетных PvE-событий +25%, ловушки и укреплённые двери |
| Агроном | производство еды +30%, расход воды −15% |
| Сборщик | приносит обычный лом по таймеру, иногда создаёт событие риска |
...
9. Старый legacy-клиент не получает новые игровые возможности. Его
   пользовательский маршрут остаётся замороженным, а Unity становится
   единственным целевым клиентом новых систем.
...
**Проверки:** `check:metadata`, `check:client-text`, `check:unity-first-run`,
`check:unity-parity`, compile-check и визуальные пробы основных экранов.
...
| Расширение живого мира | KRM-14 | причинная экономика, автономные отряды, население и видимые последствия |
