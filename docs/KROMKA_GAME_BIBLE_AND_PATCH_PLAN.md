...
- мир — граф проходимых зон, по которому ходят пешком; быстрый путь — только
  платный перенос между столицами фракций;
- личная база приватна и отсутствует среди мест мира;
...
**Назначение:** короткая боевая или сюжетная сцена точки мира; в неё входят
через портал в зоне.
...
Личная база не стоит в зонах мира, другим игрокам не видна и не может быть
захвачена.
...
|---|---|---|
| `tutorialCaravanYard` | Сборный двор №12 | за западным коридором вне зон мира; приватное обучение перед входом в Кромку |
...
| `personalBase` | Личное убежище | приватный экземпляр без места в зонах мира |
...
Миссия заканчивается выходом в зоны мира, доской контрактов, торговлей и
первой линией основного сюжета. Игрок не получает спутника и не формирует NPC-
отряд.
...
мира из зон и перемещения — 18.1.
...
существ в зонах мира. Мир продолжает этот цикл без присутствия игроков в
...
2. **Читаемость.** Карточка точки показывает состояние, главную причину и
   возможное действие игрока.
...
а семейные компоненты стабилизации остаются в лабораториях Сердцевины. Каталог
угодий обещает то, что сервер действительно кладёт на труп обитателя, и узлы
логова.
...
Каталог ищется по названию и категории, включая товары без текущих заявок.
Мгновенная сделка подтверждается с выбранным количеством и итоговой суммой.
Свои заявки можно переоценить и изменить их остаток и срок; увеличение
остатка продажи требует новой заявки. Изменение снова оплачивает сбор,
обновляет очередь и пересчитывает резерв покупки. Срок по умолчанию — 30 дней.
Перед выставлением снаряжение полностью ремонтируется. История цен строится
из сделок каждого города за сутки, 7 и 28 дней; личный журнал показывает
последние сделки, изменения, отмены и истечения срока.
...
NPC-станков нет. Станки стоят на **участках**, которыми владеет поселение, и
строят их сами игроки. Право аренды участка разыгрывается на аукционе на срок;
арендная плата сгорает. Постройка станка стоит материалов (`plots.stationCosts`),
и город помнит, кто за них платил: когда участок уходит другому игроку, станок
остаётся на месте, а строителю возвращается половина нынешней рыночной цены его
материалов (`plots.stationRefundPct`) — один раз, по книге аукционера этого
города. Новый владелец получает станок даром и может снести его, но не получит
за него ничего и строит заново с нуля. Арендатор назначает плату за пользование станком — она уходит ему, это
...
зонах мира (раздел 18.1): ходят через ворота, приходят в зоны с игроками и гибнут насовсем;
на обзорной карте их не видно.
...
Владение точкой является следствием снабжения и присутствия, а не
самостоятельным ресурсом. Для каждой точки считается давление контроля:
...
медленно и только пока в его зоне нет игроков. Зачистка надолго делает округу
...
Карта мира показывает зоны с цветом опасности и номером, открытые ворота,
места и флажок игрока — и ничего больше: других игроков, групп A-Life,
патрулей и событий на ней нет. В сцене дистанция камеры может сокращать число
подписей и частиц, но не скрывать значимые объекты.

Карточка места на странице «Мир» ПУТНИКа называет его состояние, главную
причину и доступное действие.
...
Навыка, ускоряющего путь, нет: в мире проходимых зон путь — это ходьба.
Очки, вложенные в навык, которого нет в каталоге, возвращаются персонажу при
входе. «Наука» читает довоенные системы, «Ремонт» возвращает в строй
...
| Чёрная (Сердцевина и зоны мира вокруг неё) | `pvpBlack` | разрешена | выпадает всё; каждая единица с шансом 33% становится ломом | выпавшее −20…40% | ×2,6 |
...
Режим виден на карте мира, у ворот и порталов до входа и на HUD. Столицы и
обучение всегда безопасны. Сюжетные ключи, счёт валюты, личная база и клановый склад не падают.
...
### 18.1. Пространства и мир из зон

| Пространство | Правило |
|---|---|
| Зона мира | общая MMO-комната; сверх 40 игроков — каналы той же зоны |
| Поселение/авторская локация | постоянная общая MMO-комната внутри своей зоны |
| Случайная встреча, событие, бой отрядов | временная комната точки мира, вход — портал в зоне |
...
Мир — полный граф проходимых зон, как в Ragnarok Online и Albion Online
(решение 20.09.2026). По нему ходят пешком или едут на мотоцикле из слота
«Транспорт» тем же ходом, но вдвое быстрее; пути по карте с таймером нет.

**Зоны.** Граф — 197 зон по 20 км на сетке 19 × 15
(`data/kromka/zone-graph.json`). Его собирает `node tools/build-zone-graph.js`
из географии `data/global-map.json` и играбельного контура
`data/kromka/global-map-playable.json`; ручные правки (закрытые стороны, имена)
лежат в необязательном `data/kromka/zone-graph.overrides.json`. Зона `z_CC_RR`
— обычная локация 320 × 320 м со своей сценой Unity, как у любого места:
конструктор из кусков по 40 м (`src/server/zone-builder.js`, куски —
`data/zones/chunks/`) даёт ей только первую раскладку, детерминированно по зерну
зоны, а дальше сектор правят руками (`node tools/freeze-zone.js z_CC_RR` →
`data/zones/authored/<id>.json` и «Кромка → Собрать сектор в сцену»;
`--release` возвращает сектор генератору). Зоны
пронумерованы с севера на юг и с запада на восток, имя — топоним по облику
региона с номером («Ключевская долина №148»).

**Ворота.** На каждой открытой стороне зоны — ворота в соседнюю. Шаг в проход
переводит в соседнюю зону, к её противоположным воротам. Если за воротами зона,
где при смерти выпадают вещи, первый шаг показывает её правила, второй —
переводит. После перестрелки с игроком ворота 6 с не пропускают; прошедшего
ворота 4 с не задеть, пока он сам не выстрелит; товарищи по мировому отряду в
8,5 м от шагнувшего проходят вместе с ним.

**Города — целые секторы.** Шесть столиц фракций и Ключи занимают свои секторы
целиком (`cities` в `zone-graph.overrides.json`): ворота соседней зоны ведут
прямо в город, портала внутри сектора нет, имя сектора — имя города без номера,
а правила сектора — правила самой городской локации: все семь городов мирные. Игрок входит с той
стороны, откуда пришёл, и выходит любой стороной — каждая ведёт к своему
соседу. A-Life в города не заходит.

Город собирает свой конструктор (`src/server/city-builder.js`) — тем же набором
префабов, что и зоны, и так же на лету. Сам город компактный, как в Albion:
160 × 160 м против 320 м у зоны пустоши. План один на все города, чтобы в чужой
столице человек не искал, где что: стена с воротами на каждую открытую сторону,
улицы от ворот к площади, кольцевая дорога и четыре квартала — **банк** (отдельное
здание: внутри хранилище фракции и аукционер), **рынок** (торговцы), **мастерские**
(ремонтник и станки), **жильё**. На площади — ориентир, доска работ, диспетчер
переноса, лазарет и квестовые объекты. Авторское содержимое города не пропадает:
конструктор переносит его по id в подходящий квартал, меняя только место.

**Места и порталы.** Базы, логова, заставы стоят внутри своих зон: вход —
портал места, выход — золотая полоса по краю места обратно в зону («Выход:
<зона>»). Точки мира симуляции — публичное событие, бой отрядов — стоят
порталом в зоне своей точки. Угодья оставляют в каждой зоне внутри своего
контура портал «Следы: <угодья>», и каждый вход через него разыгрывает одну
встречу. В точку входят, только стоя у портала; билет в её комнату выдаёт
сервер, а край комнаты выводит обратно в ту же зону.

**Каналы и сон.** В зоне до 40 игроков; следующие попадают в копию той же
зоны — канал (`z_CC_RR#ch2`, `#ch3`…), товарищи по мировому отряду — в один
канал.
Пустая зона через 10 минут засыпает; вскрытые тайники и выработанные ресурсы
ждут её пробуждения до конца игрового дня.

**Цвет опасности.** Зона столицы фракции мирная, восемь зон вокруг неё
(квадрат 3 × 3) синие; других синих зон нет. Остальные красят правила
`dangerCells` в `data/kromka/economy.json` по центру зоны
(`src/server/danger-cells.js`): в 22 км от Сердцевины — чёрная (сейчас пять
зон), Глухой обвод, Стеколье и Нулевая котловина — красные, остальное —
жёлтое. Цвет зоны — её режим: он задаёт PvP и потери (раздел 16.5), а
конструктор ставит по цвету больше логов, аномалий и тайников.

**Перенос между столицами.** Единственный быстрый путь — диспетчер переноса в
каждой столице фракции (`src/server/fast-travel.js`, блок `fastTravel` в
`economy.json`): другая столица за `baseFee + feePerKm × км` между зонами столиц
(сейчас от 76 до 161 марки), клан‑владелец Депо Обводного платит на 15%
меньше. Из боя (15 с после урона или выстрела) и с артефактами в рюкзаке не
отправляют: артефакты выносят из опасных зон ногами.

**Сердцевина.** В зоне Сердцевины (`z_09_07`) стоит её портал — ворота
территории. С подписанным контрактом он заводит на базу фракции игрока, без
контракта сервер предлагает выбрать фракцию, и игрок подписывает контракт тут
же. Базы стоят в своих зонах без портала, их край выводит в эту зону; в саму
центральную сцену ходят только метро своей базы.

**Карта мира.** Кнопка «КАРТА МИРА» у миникарты открывает 3D-сцену мира с
сеткой зон на рельефе: цвет, номер, открытые ворота, места и флажок там, где
стоит игрок. Клик — карточка зоны или места, «Проложить путь» — цепочка зон
через открытые ворота и подсказка под миникартой, в какие ворота идти. Других
игроков, групп A-Life, патрулей и событий на ней нет — она показывает, куда идти.

**A-Life (`worldModel.dangerEcology`,
`data/kromka/danger-ecology.json`).** Угрозы не рождаются в сцене — их
приводит мир, как в S.T.A.L.K.E.R.; досыпки врагов по таймеру нет. Группы
монстров Кромки и налётчиков живут постоянно на сетке зон: клетка экологии —
одна зона. У группы есть логово, особи со своим здоровьем, цель и счёт потерь.

| Зона | Кто живёт |
|---|---|
| синяя | банды налётчиков, стаи гари, выводки пыльников, стада фонарников |
| жёлтая | банды налётчиков, стаи гари, выводки пыльников, слухачи, стада фонарников |
| красная | налётчики, гарь, пыльники, слухачи, рыхляки, плакальщики, складни, выжженные, фонарники |
| чёрная | гарь, слухачи, рыхляки, плакальщики, складни, выжженные |

- Логова — места логов, которые конструктор ставит в зоне по цвету: синяя 1,
  жёлтая 1, красная 2, чёрная 3 (267 на мир). Логово пополняет группы медленно
  — раз в 2 ч / 90 мин / 75 мин / 45 мин реального времени от синей до
  чёрной — и только пока в его зоне нет игроков.
- Без игроков группа отдыхает у логова, обходит соседние зоны и возвращается.
  Раненые понемногу поправляются.
- Группа с чутьём слышит игроков в соседних зонах и с вероятностью своей
  агрессии идёт туда. Шаг — переход в соседнюю зону только через открытые
  ворота, поэтому пришедшая группа появляется у ворот со стороны прихода в
  каждом канале с игроками, и игроки видят предупреждение («С севера подходит:
  стая гари»).
- Группы, что уже стоят в зоне, появляются у своего логова или в зоне
  появления, когда туда приходят игроки, — в каждом канале; убитая особь
  исчезает во всех каналах сразу.
- Погибшая особь не возвращается, группа без особей исчезает. Большие потери
  уводят группу воротами, дальними от игроков. Опустевшая зона отпускает группы
  в мир с ранами выживших. Угодья (PvE-области) живут по своим правилам.
- Враждебных отрядов прежней живой пустоши нет: налётчики и звери живут в
  зонах. Встречи из `data/encounters.json` выводят существо своего вида
  (`creatureTypeId`).

**Старые сохранения.** Персонаж, сохранённый «на карте» прежнего мира или в
сцене её клетки, при входе встаёт в зону своей точки, из красной и чёрной — в
ближайшую синюю или мирную (`src/server/zone-migration.js`, для файла
сохранений — `tools/migrate-saves-to-zones.js`).

### 18.2. Мировые активности

Живая пустошь создаёт сигналы бедствия, разведку, добычу, защиту аванпоста и
штурм/диверсию. Доска работ одной кнопкой подбирает ближайшую срочную вылазку
(«Подобрать вылазку поблизости»); живой сигнал помощи идёт первым. У задачи
есть причина, место, риск, срок, лимит, общая цель и личный вклад. Награда зависит от участия; присутствие, победа NPC и reconnect её не
выдают.

### 18.3. Временная группа игроков

Запрет отряда относится к постоянным NPC-спутникам. Игроки могут временно
объединяться для похода и активности. Группа не имеет общего инвентаря и не
переносится между несвязанными задачами.

Доступны общие цели, поднятие раненого, сигнал помощи и метки
...
Зоны мира и сцены мест используют один вид сверху и одну камеру. Карта мира —
3D-сцена мира со своей камерой: сетка зон с цветами и номерами на рельефе,
флажок игрока и путь до выбранной цели.

Окружение, игроки, NPC и существа не исчезают из-за дистанции камеры. Можно
упрощать подписи, частицы, тени и анимацию, но не значимый объект. Ворота зоны,
порталы и край места читаются издалека, а правила зоны за ними видны до входа.
...
- не стоит в зонах мира и отсутствует в публичном снимке мира;
- войти можно из безопасного поселения;
...
- в зонах мира стоят восемь фиксированных клановых баз из раздела 4.3;
...
| Депо Обводное | клановый склад +50%, перенос между столицами дешевле на 15% |
...
2. Камера сверху и текущая система прицеливания сохраняются. Мир — граф
   проходимых зон (раздел 18.1): по нему ходят пешком через ворота, места
   открываются из своих зон, а быстрый путь — только платный перенос между
   столицами фракций.
...
безопасном хабе Сердцевины со входами из чёрных сцен; мир делится на
проходимые зоны, как в Albion; транспорт — позже. Правило KRM-03 «синь не валюта» заменено.
...
5. **Опасные земли.** Цвет опасности у каждой зоны мира, чёрные зоны вокруг
   Сердцевины; угрозы приводит A-Life, а не серверный спавн.
...
**Состояние:** все шесть этапов выполнены; сверх них сделаны мир из зон
(раздел 18.1), A-Life в зонах, марки на счёте аккаунта и переключаемый фокус.
Остаётся платёжный магазин сини.
...
| Игрок не понимает риск PvP-точки | цвет зоны виден на карте мира, правила — у ворот и порталов до входа и на HUD |
