...
самой Сердцевины) сквозными не считаются — к месту можно дойти. Врагов в
сцены приводит A-Life (итерация 3): досыпки угроз по таймеру нет.
...
**Итерация 3 — A-Life (реализовано, `worldModel.dangerEcology`,
`data/kromka/danger-ecology.json`).** Угрозы не рождаются в сцене — их
приводит мир, как в S.T.A.L.K.E.R. Группы монстров Кромки и налётчиков живут
...
- Враждебные отряды прежней живой пустоши сняты (`worldModel.hostileWorldParties
  = false`): налётчики и звери теперь живут в клетках. Встречи из
  `data/encounters.json` спавнят существо своего вида (`creatureTypeId`), а
  не случайный тип.
...
**Видимость на карте и клетки Сердцевины (реализовано).** «Странник»
...
выходами к соседям, заливка карты переделана и получила легенду
(`check:account-sin`, `check:danger-cells`). A-Life опасных клеток: группы
монстров Кромки и налётчиков живут на сетке мелких клеток, приходят в сцены
с края и гибнут насовсем (`check:danger-ecology`). «Странник» задаёт, кого
игрок видит на карте и насколько заметен сам; клетки Сердцевины получили
номера на карте и в сцене, а миникарта — кнопку «КАРТА МИРА» с флажком
игрока (`check:danger-ecology` → `check-map-sightings-network`).
