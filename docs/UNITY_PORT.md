...
- Пластинки в эфире: `npm run fetch:radio` (tools/radio-library.py) качает
  ...
  крутит перемешанный плейлист через `UnityWebRequestMultimedia` (MPEG,
  работает и в WebGL), следующий трек подгружается за 8 с до конца, между
  пластинками пауза 1.5–3.5 с; «♪ Сейчас играет» — отдельная карточка в блоке
  ЭФИР. Синтезированных звуков (шум, ноты маяка, пакеты, тики, джинглы)
