...
пустоши — живёт в `server.js` и `src/server/`. Клиент отвечает только за
...
| Слой | Объём | Судьба |
|---|---|---|
| `server.js` + `src/server/` | ~38 400 строк | **Не трогаем.** Unity подключается к нему как обычный клиент |
...
| `public/js/game/` — Three.js клиент | ~52 700 строк | **Перенесён** на C# и удалён |
...
Побочный эффект такого разделения: web-клиент продолжал работать всё время
переноса, и обе версии ходили в один мир одновременно. После перехода игроков
на Unity WebGL прежний клиент удалён.
...
WebGL-сборка появилась позже и стала основной поставкой для игроков (см.
«Ограничение WebGL» и «Деплой: Unity — единственный клиент» ниже).
...
320×360 и получил 19 313 отличимых пикселей. Проверка сетевой и предметной чётности, пока
web-клиент был в репозитории, сверила с ним 43 исходящих и 43 входящих события,
58 предметов, 18 рецептов, 19 модификаций оружия и шесть стартовых перков.
...
| Глобальная карта | `RoaGlobalMap.cs`, `RoaUnityGlobalMapScene.cs`, `GlobalMapAuthored.unity` | prefab-only маршрут `wasteland → карта → settlement`, территории, активности и 3D-модели проверены в Windows player |
...
Управление перенесено из web-клиента: **персонаж смотрит на курсор, а ходит
...
просто подмешивал клипы библиотеки к базовой модели. Legacy-анимация Unity
...
записала свой кадр. В `Update()` их бы затёрло. Web-клиент выдерживал тот же
...
таблица «передних осей» — из `00a_actor_facing.js:6` (теперь
`src/server/actor-facing.js`). Она используется для
...
### NPC Combat & Readability 4.7

- Unity применяет из каждого `enemyFrame` не только позицию, но и HP,
  `aiState`, смерть, обыск и viewer-specific враждебность. Поэтому урон другого
  игрока сразу меняет полосу здоровья и запускает одну реакцию попадания.
- Подтверждённый урон даёт серверный hit-stagger 130–310 мс с иммунитетом между
  реакциями. NPC не скользит через hurt-позу, но очередь автоматического оружия
  не может держать его в бесконечном stun-lock.
- Смерть атомарно очищает скорость, цель, взгляд, подготовку атаки, перезарядку
  и путь. Клиент дополнительно защёлкивает HP=0 и приоритет death над любым
  поздним volatile-кадром.
- Строка над HP не получила нового ряда: в том же месте показывается
  `ВРАГ · ФРАКЦИЯ`, `В БОЮ · ФРАКЦИЯ` или `НА ВАС · ФРАКЦИЯ`.

## Combat Flow 4.8

- Humanoid и legacy-существа используют один приоритет тела:
  `death > reaction > attack > locomotion > idle`. Поздняя атака больше не
  обнуляет hit-stagger, а death-клип запускается через `PlayMode.StopAll`,
  поэтому за трупом не остаётся вес ходьбы.
- `attackMs` задаёт длительность замаха NPC: общая фаза `StrikeContactPhase`
  достигается к приходу `enemyMelee/enemyAttack`. Дальний NPC не показывает
  отдачу в начале предупреждения — recoil запускается самим событием `shot`.
- `enemyKilled` и NPC-ветка `playerDamaged` несут `sourceX/sourceZ`. Последний
  подтверждённый источник выбирает один из восьми yaw-вариантов death-клипа,
  поэтому персонаж падает от удара, а не всегда в одну экранную сторону.
- Подтверждённый hit остаётся в фиксированном пуле, но теперь различает
  баллистику/лазер/плазму цветом, добавляет короткую наземную пыль и ограниченный
  дистанцией импульс камеры. Новых HUD-панелей патч не создаёт.

## Visual Cohesion 4.9

- Три масштаба стратегической карты больше не переключают отряды, места и
  угрозы одним кадром. Видимость слоя меняется за 180 мс через спокойный
  scale-in/scale-out; территориальная подложка остаётся мгновенной, чтобы между
  клетками никогда не появлялись щели.
- Авторская карта получила собственный стратегический световой профиль:
  тёплый верхний свет, сдержанные отражения, тёмное основание и линейный туман
  только за игровым центром. Цвет тумана близок к фону камеры, поэтому край
  диорамы растворяется в одном поле вместо серой пустоты.
- HUD сохраняет состав элементов, но меняет визуальную громкость по контексту.
  В исследовании оружейная строка и миникарта спокойнее, активность освобождает
  внимание карточке цели, а бой возвращает карте и оружию полный контраст.
- `Canvas.pixelPerfect` остаётся обязательным; все тексты основного HUD и
  глобальной карты получают одинаковую тень в один физический пиксель. Это
  защищает кириллицу на пёстром мире и при переходе в полноэкранный режим без
  размытой многопиксельной обводки.

## NPC Coordination 5.0

- Кольцевые позиции ближнего боя теперь отделены от права на сам удар. В
  зависимости от размера группы одновременно совершают выпад не более одного,
  двух или трёх NPC; остальные держат собственные позиции и ждут короткого
  персонального окна. После удара исполнитель получает стабильную паузу, поэтому
  очередь естественно переходит к соседям и не застревает на первых бойцах.
- Серверное состояние `pressure` означает «в позиции, ищет момент». Оно не
  создаёт ложный телеграф атаки, не двигает корень модели и остаётся боевым
  прерыванием расписания NPC. Отступление, hit-stagger, смена цели и смерть
  атомарно освобождают занятое окно.
- Строка над HP показывает не только отношение и фракцию, но и наблюдаемое
  намерение: `СБЛИЖАЕТСЯ`, `ИЩЕТ МОМЕНТ`, `АТАКУЕТ`, `ПЕРЕЗАРЯЖАЕТСЯ`,
  `ОТСТУПАЕТ`. Личная дальняя угроза отличается от ближней как `ЦЕЛИТСЯ В ВАС`
  и `АТАКУЕТ ВАС`, не добавляя новой HUD-панели.
- Смерть остаётся терминальным состоянием: поздний volatile-кадр не возвращает
  ходьбу, а сервер очищает движение, цель, телеграф, путь и боевую резервацию.
...
Unity загружает те же 57 GLB-представлений, что загружал web-клиент: 24 узла из
...
Web-HUD был собран из готовых артов с процентным позиционированием текста поверх —
...
- **Фракции** — текущая сторона и три основные фракции: Старый Клим,
  Свалочный союз и Орден Ретранслятора. Только к ним можно присоединиться, и
  только для них ведётся репутация. Вольные караваны и нейтральные жители
  показаны как независимые без шкалы репутации, а рейдеры, супермутанты и дикие
  существа — как угрозы без числового отношения к игроку. Внутренние восемь
  групп сохраняются в симуляции для владения точками, экономики, маршрутов и
  боёв, но не считаются восемью игровыми фракциями.
...
- `RoaGlobalMapCanvas.cs` — сайдбар #global-map-window .global-map-side
  ...
  выключена `RoaGlobalMap.CanvasDriven`; названия столиц и до трёх приоритетных
  активностей рисует отдельный ограниченный пул Canvas-подписей. Цвет показывает
  тип вылазки, выбранная цель усиливается, а детерминированная раскладка не пускает
  подписи на сайдбар, кнопку сигналов, соседние метки и мобильную подсказку. Фон и
  текст не перехватывают мышь/touch; позиция обновляется в `LateUpdate` после камеры.
  HUD на карте скрыт (web-окно карты перекрывает всё), ⚙ сдвигается левее сайдбара.
...
## Многоэтапный HUD локальных активностей

- `RoaWorldActivityCanvas` больше не сжимает серверный массив `objectives` в
  одну меняющуюся строку. Ограниченный пул из трёх Canvas-строк одновременно
  показывает завершённые, текущий и следующий этап; элементы не создаются при
  каждом снимке и не перехватывают управление.
- У сигнала бедствия сначала активен поиск маяка, а зачистка засады помечена как
  следующий этап. У операции до решения видны обе закрытые ветки «Штурм» и
  «Диверсия»; после выбора проигранная ветка исчезает, выбранная становится
  текущей. Одноэтапные разведка, добыча и оборона используют тот же контракт.
- Основная, бонусная и максимальная границы различаются текстом и цветом. Когда
  сервер открывает завершение, добыча ради бонуса остаётся видимой, но отдельная
  текущая строка прямо говорит «Добраться до эвакуации», «Завершить оборону» или
  «Завершить спасение». Авторитетный прогресс подсвечивает именно текущую строку.
- `WorldObjectiveLabels` связывает этот план с локацией: до четырёх ближайших
  активных маяков получают экранные подписи с авторским именем, расстоянием,
  состояниями «ДОСТУПНО»/«ГОТОВО» и отдельным выходом. Они обновляются в
  `LateUpdate`, обходят основной HUD, компас, safe area и друг друга, не
  перехватывают ввод. Закрытые точки будущей ветки операции больше не рисуются
  ни в мире, ни на мини-карте; после выбора диверсии сервер переводит их в
  `pending`, и только тогда они становятся видимыми.
...
- Пластинки в эфире: `npm run fetch:radio` (tools/radio-library.py) качает
  библиотеку в `radio-library/raw/` (Викисклад PD, archive.org mrcsf-music и
  Great 78 Project, ручные папки `pixabay/` и `itch/`), `npm run build:radio`
  перекодирует всё в моно MP3 64 kbps и пишет `public/radio/manifest.json`
  (название · исполнитель · год · источник · лицензия · `tier`). У каждого
  трека ровно одна станция, чтобы каналы звучали по-разному: безопасность —
  военное (ансамбли Красной армии, марши, гимны, песни войны), маяк —
  народное и бытовое (пляски, баян, балалайка, стилизации pixabay/itch),
  пепел — остальное (романсы, арии, симфоническое, дореволюционные и
  неразборчивые пластинки). Оба каталога вне git — на VPS `public/radio/`
  собирается при деплое. `RoaRadio` берёт манифест с BaseUrl, на живом канале
  играет по **общему расписанию**: порядок пластинок станции детерминирован
  (`BuildOrder`, seed из манифеста `scheduleSeed` + номер канала), слоты
  «длительность + пауза 2.5 с» идут непрерывным циклом от `scheduleEpoch`
  (2026-01-01Z), текущий слот и смещение считаются от серверных часов
  (поправка из заголовка `Date` ответа с манифестом), поэтому все игроки
  слышат одну запись с одного места, а переключение канала попадает в
  середину идущей пластинки. Клип грузится целиком через
  `UnityWebRequestMultimedia` (MPEG, работает и в WebGL) и встаёт на нужное
  смещение, дрейф больше 1.2 с подтягивается раз в 2 с, следующая пластинка
  подгружается за 8 с до смены слота; «♪ Сейчас играет» и «Далее в эфире» —
  карточки в блоке ЭФИР. Синтезированных звуков (шум, ноты маяка, пакеты, тики, джинглы)
  больше нет: без манифеста (404) приёмник молчит, строка сигнала сообщает
  «Библиотека эфира недоступна», повтор запроса раз в минуту. Лицензии разные
  (PD; CC BY-NC-ND у оцифровок mrcsf; не указана у Great 78) — поле `tier`
  манифеста для ревизии перед релизом.
...
- `RoaItemInfo.cs` — сгенерированный из `ITEMS` web каталог: описание,
  ...
  пригодность к использованию. Генератор `tools/build-unity-item-info.js` читал
  каталог браузерного клиента и удалён вместе с ним; таблица правится в самом
  `RoaItemInfo.cs`.
...
Web-клиент намеренно фиксировал время на **16:12** и прямо отключал ночной
штраф обзора. Unity по умолчанию сохраняет это поведение, а не связывает
...
всегда использует полную скорость персонажа, как web-клиент. Справа
...
Автоцель выбирается из живых NPC, которые разрешены тем же
`RoaVisibilityGate`, и обводится отдельным кольцом. В зоне, где сервер
разрешает PvP (`pvp`, `pvpEvent`, `pvpFullDrop`), в тот же цикл по дистанции
входят игроки (`RoaRemotePlayers.CollectMobileTargets`, id с префиксом
`player:`): без лежачих, без своей и союзной фракции (Управа и Лига трактов),
без друзей, соклановцев и спутников по отряду каравана (`worldPartyId`), без
столиц фракций. В Сердцевине цикл повторяет `serverTerritoryPvpBlock`: не
предлагает подписавших тот же контракт (`territoryFactionId`), молчит, пока
стрелок стоит на платформе фракции, и не предлагает цель на её собственной
платформе (сервер защищает её вне боя). Зону подтверждают и `self`, и HUD;
после серверного переноса (осада, засада) сервер присылает свежий `self`.
Выбранный игрок
обстреливается по живой позиции, а не по снимку списка. С аптечкой в руке
игроки в цикл не входят: их лечат из ПУТНИКА с согласия. Без выбранной цели
огонь идёт на 18 м по текущему направлению.

Ракетница на телефоне стреляет коротким тапом по миру (не дольше 0,35 с и
не дальше 18 px от места касания, то есть короче удержания метки активности
0,40 с): точка под пальцем ограничивается дальностью оружия и не ставится
ближе 6,3 м к стрелку — радиус взрыва 4,2 м плюс запас. Тап над кнопками, в
зоне стика, во время броска болта и тап, закрывающий меню меток,
игнорируется; в безопасной и мирной зоне режим выключен. Сенсорный режим отключает наведение по
...
одновременных pointer id. Отдельный блок проверяет фильтр игроков в автоцели
(PvP-зона, союзники, друзья, клан, отряд каравана, контракт и платформы
Сердцевины на настоящей `coreZone.json`, лежачие, дальность, префикс id) и
безопасную дистанцию тапа ракетницей. Скрытый player в принудительном mobile-режиме также
...
- `GET /api/global-map` загружает те же 900 авторских ячеек, четыре столицы и
  пять линий инфраструктуры, которыми пользовался web-клиент;
...
web-клиент строил эти значки из примитивов, а не загружал GLB, поэтому Unity
...
если известная клиенту комната пуста. Web-клиент отбрасывал только когда обе
стороны знают комнату и они разные
(`public/js/game/05_multiplayer_core_state.js:390`); Unity перешёл на то же правило.
...
- 20 авторских досок работ открывают актуальные `worldTasks` из
  ...
  можно вступить в одну из трёх основных фракций через `worldFactionJoin`:
  Старый Клим, Свалочный союз или Орден Ретранслятора. Сервер снова проверяет
  место, фракцию и отсутствие активных обязательств. Независимые контракты
  караванщиков не требуют членства и не изменяют фракционную репутацию.
...
Курсорный выбор перенесён из web-клиента буквально: направление берётся от
...
- Авторитетные `activityType`, `activityPhase` и `visualAction` теперь доходят до
  видимой Unity-презентации, а не остаются только сетевыми полями. Неподвижные
  жители различимо работают, торгуют, несут пост, общаются, едят и отдыхают.
- Процедурные жесты затрагивают только корпус, голову и руки. Ноги остаются под
  локомоцией и foot IK; движение, попадание, атака, тревога и смерть имеют строгий
  приоритет над бытовой позой.
- Фаза жеста детерминированно различается по id NPC, поэтому группа рабочих или
  охраны не двигается синхронно. Новых сетевых тиков и тяжёлых Animator-графов нет.
- Строка над HP показывает текущий смысл: `РАБОТАЕТ`, `ТОРГУЕТ`, `НА ПОСТУ`,
  `ОБЩАЕТСЯ` или `ИДЁТ К МЕСТУ`. При тревоге она немедленно становится `ТРЕВОГА`,
  а после снятия серверного interrupt снова отражает обычный распорядок.

## Journey Flow 5.2

Путь от глобальной карты до активности теперь считается одной восстанавливаемой
операцией, а не набором несвязанных экранов:

- любой `EmitWithAck` Unity завершается ответом, локальным таймаутом или отказом
  из-за разрыва связи; интерфейс больше не остаётся навсегда в состоянии ожидания;
- подтверждённое прибытие создаёт серверный билет `pendingWorldDrop` на 90 секунд
  и сохраняет его вместе с персонажем; до успешного `changeLocation` персонаж
  остаётся на глобальной карте;
- после reconnect Unity восстанавливает цель из билета и автоматически повторяет
  вход до четырёх раз с ограниченной растущей задержкой; затем оставляет игроку
  понятную ручную кнопку повторения;
- повторный `changeLocation` идемпотентен: если сервер уже выполнил переход, но ACK
  потерялся, повтор возвращает актуальный baseline комнаты вместо ложного отказа;
- после фактического входа сервер сначала сохраняет погашенный билет и новую
  комнату, и только затем отвечает клиенту;
- устройство и схема управления передаются корректно и для ПК, и для мобильного
  клиента.

Проверка: `npm run check:unity-journey-flow`. В Unity тот же контракт доступен через
`Realm of Ashes/Проверить Journey Flow 5.2` и включён в общий client audit.

## Mission Director 5.3

Локальная активность теперь имеет серверного режиссёра темпа и восстановления:

- пустая или устаревшая активность комнаты больше не скрывает новое задание,
  которое прибывший игрок принял и отслеживает на глобальной карте;
- оборона сразу получает играбельную первую волну, сигнал бедствия выпускает
  засаду только после активации маяка, а операция начинает боевую волну только
  после выбора прямого штурма; диверсия не превращается в принудительный бой;
- число живых противников поддерживается относительно остатка цели и размера
  временного отряда. Исчезнувшая или застрявшая волна автоматически заменяется,
  поэтому счётчик убийств всегда достижим вплоть до авторского максимума;
- отсутствующая, заблокированная или оказавшаяся в непроходимой геометрии точка
  разведки, маяка, подхода либо диверсии восстанавливается на безопасной позиции;
- Unity объявляет выбор ветки, открытие эвакуации и восстановление цели, а на
  последней минуте и последних 15 секундах показывает отдельное предупреждение;
- `director`, `lastProgressAt` и `stageStartedAt` входят в авторитетный lifecycle,
  но клиент по-прежнему не создаёт врагов и не засчитывает прогресс сам.

Проверка: `npm run check:unity-mission-director`. В Unity: `Realm of Ashes/Проверить
Mission Director 5.3`; probe включён в общий client audit.

## Encounter Layout 5.4

Каждая локальная активность получает детерминированную пространственную схему:

- сервер выбирает безопасный центр операции, компактные границы для целей и до
  четырёх линий подхода по сторонам локации;
- точки разведки, маяка, выбора подхода и диверсии создаются и восстанавливаются
  внутри одного района, а не распределяются по всей локальной карте;
- боевые цели делятся на три читаемые волны. После порогов прогресса режиссёр
  меняет линию подхода, а `spawnServerEnemy` обязан найти место рядом с выбранной
  линией и не может скрыто откатиться к случайной клетке;
- если игрок перекрыл линию, сервер пробует другую авторскую линию и сразу
  публикует изменившееся направление;
- Unity показывает `ВОЛНА N/3 · НАПРАВЛЕНИЕ` в HUD, красную границу подхода в
  мире, отдельный красный threat-маркер на миникарте и полноэкранной карте;
- район целей объединён тонкой collider-free границей на земле. Она не занимает
  дополнительную постоянную панель HUD и не вмешивается в движение или бой.

Проверка: `npm run check:unity-encounter-layout`. В Unity: `Realm of Ashes/Проверить
Encounter Layout 5.4`; probe включён в общий client audit.
