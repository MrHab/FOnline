...
блокируются движение, бой, взаимодействие и быстрые слоты; мир на сервере при
этом продолжает жить.
...
- **Сборка**: меню «Кромка → Build WebGL» (`Assets/Editor/RoaWebGlBuild.cs`)
  ...
  Settings: сцены мест грузятся аддитивно, и без них сборка молча
