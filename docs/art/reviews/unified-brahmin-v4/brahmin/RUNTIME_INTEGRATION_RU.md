...
Корневой масштаб компенсирован покомпонентно
(`runtimeScaleCompensation` в `tools/build-approved-creature-models.js`),
поэтому статические габариты каталога равны:
...
- рендеры: `renders/actions/`;
- отчёт: `runtime-action-review-report.json`;
- гейт приёмки и габариты каталога проверяет `npm run check:approved-creatures`.
