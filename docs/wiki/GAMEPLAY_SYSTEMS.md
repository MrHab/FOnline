...
(строка пути или `{ prefab, tint, paint }`: `tint` умножает цвет серых мешей —
так руда одного камня различается по тирам, `paint` закрашивает атлас целиком —
так у шкур и ткани нет рисунка ковра и надписей):
