...
У каждого тира свой цвет (`tiers[].color`): T1 бронза, T2 зелёный, T3 синий,
T4 фиолетовый, T5 золото. У каждого семейства в `visuals` по префабу
PolygonApocalypse на тир для точки добычи, сырья и полуфабриката; текстура модели
не меняется, тир показывает небольшая метка его цвета (`RoaTierMark`): изолента
на рукояти или древке оружия и инструмента, обмотка на стволе, бочке или пучке
травы, пятно краски на камне, мешке, листе или шлеме. Место метки редактор
считает по треугольникам префаба (`RoaTierMarkLayout`) и хранит в палитре: в
сборке вершины моделей пака не читаются. Метка есть везде, где модель видна: точки
добычи, предметы на земле, оружие в руках у игрока и врагов, шлем на голове; у
значков тира — цветная точка в углу. Надетый комплект брони — модель персонажа
целиком, её не метим. Палитру собирает «Realm of
Ashes/PolygonApocalypse/Rebuild runtime model palette», иконки материалов —
«Напечь рендеры материалов тиров», листы ресурсов и экипировки — проба «Tiers:
resource sheets» (`Library/TierScreen`).
