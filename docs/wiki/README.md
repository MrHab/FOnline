...
Актуально для **v7.76.5**. Последняя сверка с кодом: **26 июля 2026 года**.
...
- Клиент всегда находится в одном из режимов `server`, `offline-local` или
  `blocked`: потеря авторитетной сетевой сессии очищает устаревшее состояние
  комнаты и не разрешает локально продолжить серверные игровые действия.
  Актуальный `401` не превращает уже открытый online-мир в local fallback;
  настоящий мир без server-character сохраняет режим `offline-local`.
- Join действует только в полной привязке token + character +
  `clientInstanceId` + socket + lease; ожидающий старый контекст не может
  завершиться входом другого персонажа.
- Logout и смена персонажа отменяются до disconnect, если финальный save не
  подтверждён. Leaderboard публикуется отдельно и не входит в авторитетный
  commit.
...
- Членство в караване или патруле существует только вместе с активной принятой
  ...
  снимается при cancel, terminal-переходе или смерти. Патруль завершается после
  шести часов каждого текущего участника, а награду разрешает только
  версионированный snapshot точного задания и серверная пара
  аккаунт/персонаж.
