  ...
  // The server simulation advances every five seconds. Fetching the same full
  // world projection more often only repeats a large JSON parse and competes
  // with Socket.IO traffic; party markers extrapolate between sampled ticks.
  ...
  function normalizeClientGlobalMapConfig(raw = {}) {
    ...
    const nodes = nodesSource.slice(0, 80).map((node, index) => {
      ...
      return {
        ...
        capital: node?.capital === true || !!capitalFaction || ['settlement', 'scrapTown', 'relayStation'].includes(id),
        capitalFaction,
  ...
  function globalMapCapitalNodes() {
    return GLOBAL_MAP_NODES.filter(node => node && (
      node.capital === true ||
      node.capitalFaction ||
      ['settlement', 'scrapTown', 'relayStation'].includes(String(node.id || ''))
    ));
  }
  ...
  function globalMapWorldSiteVisible(site = null) {
    ...
    if (site.capital === true || ['settlement', 'scrapTown', 'relayStation'].includes(String(site.id || ''))) return true;
