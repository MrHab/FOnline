  function globalMapWorldContacts(limit = 10, options = {}) {
    void limit;
    void options;
    return [];
  }

  function globalMapPointCoveredByWorldContact(point = null, radius = 13) {
    void point;
    void radius;
    return false;
  }

  function globalMapWorldContactAt(x, y, radius = 8) {
    void x;
    void y;
    void radius;
    return null;
  ...
  function globalMapWorldContactMeta(contact = {}) {
    const kind = String(contact.kind || '').toLowerCase();
    if (kind === 'caravan') return { label: 'Караван', glyph: '!', color: '#ffd077', bg: 'rgba(42,28,8,0.9)' };
    if (kind === 'raid') return { label: 'Налёт', glyph: '!', color: '#ff7254', bg: 'rgba(42,10,6,0.92)' };
    if (kind === 'siege') return { label: 'Осада', glyph: '◆', color: '#ff9a76', bg: 'rgba(42,12,6,0.92)' };
    if (kind === 'battle') return { label: 'Стычка', glyph: '×', color: '#ffb45f', bg: 'rgba(42,18,6,0.9)' };
    if (kind === 'lair') return { label: 'Логово', glyph: '!', color: '#ff4f3f', bg: 'rgba(42,8,5,0.94)' };
    if (kind === 'aftermath') return { label: 'Следы', glyph: '?', color: '#c8f0ff', bg: 'rgba(8,28,38,0.88)' };
    return { label: 'Событие', glyph: '!', color: '#efd078', bg: 'rgba(32,24,8,0.88)' };
  }

  function globalMapWorldContactTitle(contact = {}) {
    const meta = globalMapWorldContactMeta(contact);
    const title = String(contact.title || '').trim();
    if (!title) return meta.label || 'Событие пустоши';
    const label = String(meta.label || '').trim();
    if (!label || title.toLowerCase().includes(label.toLowerCase())) return title;
    return `${label}: ${title}`;
  }

  function globalMapWorldContactKindText(contact = {}) {
    const meta = globalMapWorldContactMeta(contact);
    const faction = String(contact.targetFaction || contact.faction || contact.details?.threatFaction || '').trim();
    return [meta.label || 'Событие', faction ? globalMapFactionLabel(faction) : ''].filter(Boolean).join(' · ');
  }

  function globalMapWorldContactDescription(contact = {}) {
    const kind = String(contact.kind || '').toLowerCase();
    const details = contact.details && typeof contact.details === 'object' ? contact.details : {};
    const text = String(contact.text || '').trim();
    const lines = [];
    if (text) lines.push(text);
    else if (kind === 'lair') lines.push('Постоянное логово в пустоши. Внутри может быть группа врагов, добыча и следы их активности.');
    else lines.push('Живое событие пустоши. Можно вмешаться лично или дать миру решить исход.');

    if (kind === 'lair') {
      const threatName = String(details.threatName || contact.title || '').trim();
      const faction = String(contact.targetFaction || contact.faction || details.threatFaction || '').trim();
      const parts = [];
      if (threatName && !text.toLowerCase().includes(threatName.toLowerCase())) parts.push(`группа: ${threatName}`);
      if (faction) parts.push(`фракция: ${globalMapFactionLabel(faction)}`);
      const members = Math.round(Number(details.members || 0));
      const strength = Number(details.strength || 0);
      if (members > 0) parts.push(`${members} существ`);
      if (strength > 0) parts.push(`сила ${Math.round(strength)}`);
      if (parts.length) lines.push(`Опасность: ${parts.join(', ')}.`);
      if (!/после зачистки/i.test(text)) {
        const respawnHours = Math.round(Number(details.fixedLairRespawnHours || details.respawnHours || 0));
        lines.push(respawnHours > 0
          ? `После зачистки логово обновится примерно через ${respawnHours} игровых часов.`
          : 'После зачистки логово обновляется примерно через игровые сутки.');
      }
    }
    return lines.filter(Boolean).join(' ');
  }

  function globalMapWorldContactExtraRows(contact = {}) {
    const details = contact.details && typeof contact.details === 'object' ? contact.details : {};
    const rows = [];
    const faction = String(contact.targetFaction || contact.faction || details.threatFaction || '').trim();
    if (faction) rows.push(['Фракция', globalMapFactionLabel(faction)]);
    const threatName = String(details.threatName || '').trim();
    if (threatName) rows.push(['Группа', threatName]);
    const members = Math.round(Number(details.members || 0));
    const strength = Math.round(Number(details.strength || 0));
    if (members > 0 || strength > 0) {
      rows.push(['Сила', [members > 0 ? `${members} ед.` : '', strength > 0 ? `${strength}` : ''].filter(Boolean).join(' · ')]);
    }
    const radius = Math.round(Number(contact.radius || 0));
    if (radius > 0) rows.push(['Зона', `${radius} точек`]);
    rows.push(['Действие', globalMapWorldContactButtonText(contact)]);
    return rows;
  }

  function globalMapWorldContacts3DSignature(rows = []) {
    return rows.map(row => [
      row?.id || '',
      row?.kind || '',
      row?.title || '',
      ...
      Math.round(Number(row?.displayX ?? row?.x ?? 0) * 10) / 10,
      Math.round(Number(row?.displayY ?? row?.y ?? 0) * 10) / 10,
      Math.round(Number(row?.priority || 0)),
      row?.encounterId || '',
      row?.zoneId || '',
      row?.roomId || '',
      row?.status || '',
  ...
  function buildGlobalMapWorldContactMarker(contact = {}) {
    const meta = globalMapWorldContactMeta(contact);
    const priority = Math.max(1, Math.min(5, Number(contact.priority || 1)));
    const color = parseInt(String(meta.color || '#efd078').replace('#', ''), 16) || 0xefd078;
    const group = new THREE.Group();
    const CircleGeometry = THREE.CircleBufferGeometry || THREE.CircleGeometry;
    const explicitRadius = Number(contact.radius || 0);
    const radius = explicitRadius > 0 ? Math.max(0.55, Math.min(1.35, explicitRadius / 9)) : 0.58 + priority * 0.06;
    const ring = new THREE.Mesh(
      new THREE.TorusBufferGeometry(radius, priority >= 4 ? 0.038 : 0.028, 8, 54),
      new THREE.MeshBasicMaterial({ color, transparent: true, opacity: priority >= 4 ? 0.68 : 0.48, depthTest: false })
    );
    ring.rotation.x = Math.PI / 2;
    const glow = new THREE.Mesh(
      new CircleGeometry(0.34 + priority * 0.025, 32),
      new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.18, depthTest: false, depthWrite: false, side: THREE.DoubleSide })
    );
    glow.rotation.x = -Math.PI / 2;
    glow.position.y = 0.035;
    const beacon = new THREE.Mesh(
      new CircleGeometry(0.14 + priority * 0.014, 24),
      new THREE.MeshBasicMaterial({ color, transparent: true, opacity: priority >= 4 ? 0.92 : 0.78, depthTest: false, depthWrite: false, side: THREE.DoubleSide })
    );
    beacon.rotation.x = -Math.PI / 2;
    beacon.position.y = 0.055;
    group.add(ring, glow, beacon);
    group.userData.contactRing = ring;
    group.userData.contactBeacon = beacon;
    group.userData.contactPriority = priority;
    return group;
  }
