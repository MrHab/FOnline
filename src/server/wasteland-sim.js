...
const WORLD_PARTY_SPEED_FLOORS = {
  caravan: 26,
  support: 30,
  patrol: 36,
  raider: 34,
...
function ensureUniqueWorldSiteLocalProfiles(sites = {}) {
  ...
  for (const site of rows) {
    ...
    if (!oldProfileIsValid) {
      let slot = stableWorldSiteHash(`${site.id}:local-size`) % poolSize;
...
function addStockpile(target = {}, source = {}, mul = 1) {
  for (const [key, value] of Object.entries(source || {})) {
    const amount = Math.max(0, Number(value || 0) * Number(mul || 1));
...
function createWastelandSimulation(options = {}) {
  ...
  function partyMeetingContact(party = {}, threat = null) {
    const stateKey = String(party?.state || '').toLowerCase();
    if (!party || party.destroyed || stateKey === 'destroyed' || stateKey === 'engaged' || stateKey === 'onsite') return null;
    const kind = String(party.kind || '').toLowerCase();
    const hostileParty = partyIsHostileEncounterParty(party);
    if (hostileParty) return null;
    if (kind !== 'caravan' && kind !== 'patrol') return null;
    const danger = threat || partyThreatInfo(party);
    const locationId = partyMeetingLocationId(party);
    const faction = party.faction || '';
    const contactKind = kind === 'patrol' ? 'patrol' : 'caravan';
    let text = kind === 'patrol'
      ? 'Отряд патрулирует дорогу. Можно войти в одну живую сцену этого патруля, поговорить с патрульными или пройти мимо.'
      : 'Караван остановился на дороге. Можно войти в одну живую сцену этого каравана, поговорить с караванщиком и поторговать.';
    if (hostileParty) {
      text = 'Враждебный бродячий отряд заметил вас в пустоши. Вход ведет в живую сцену этой группы; после победы отряд исчезнет с глобальной карты.';
    }
    return {
      id: `party_${party.id}`,
      kind: contactKind,
      title: party.name || (kind === 'patrol' ? 'Патруль пустоши' : 'Караван пустоши'),
      text,
      x: party.x,
      y: party.y,
      radius: 9,
      priority: Number(danger.riskLevel || 0) >= 48 ? 3 : 2,
      sourceType: 'party',
      sourceId: party.id,
      partyId: party.id,
      faction,
      ...
      pvpMode: 'pvp',
      actionLabel: hostileParty ? 'Вступить в бой' : (kind === 'patrol' ? 'Подойти' : 'Войти'),
      details: {
        partyMeeting: true,
  ...
  function ensureSiteConflictZone(site = {}) {
    const conflict = activeSiteConflict(site);
    if (!site || !site.id || !conflict) return null;
    if (isCapitalProtectedSite(site)) return null;
    const primaryFaction = siteConflictPrimaryFaction(site);
    const id = safeId(`site_conflict_zone_${site.id}`, `site_conflict_zone_${Math.round(Number(site.x || 0))}_${Math.round(Number(site.y || 0))}`);
    const locationId = String(site.locationId || locationIdForWorldContact({ kind: 'raid', targetFaction: primaryFaction })).slice(0, 80);
    const actors = [
      ...siteDefenderActors(site),
      ...siteAttackerActors(site)
    ].map((actor, index) => normalizeBattleActor(actor, index, state.worldHour)).filter(Boolean);
    return upsertWorldZone({
      id,
      kind: 'raid',
      ...
      sourceType: 'site_conflict',
      sourceId: site.id,
      siteId: site.id,
      faction: site.owner || '',
      targetFaction: primaryFaction,
      encounterId: encounterIdForWorldContact({ kind: 'raid', targetFaction: primaryFaction }),
      locationId,
      roomId: sharedRealityRoomId(locationId),
      pvpMode: site.pvpMode || 'pvp',
      expiresHour: conflict.expiresHour || site.raidUntil || 0,
      details: {
        simBattle: true,
        realTimeBattle: true,
        simulationDisabled: true,
        ...
        battleState: 'active',
        siteId: site.id,
        conflictId: conflict.id || siteConflictTaskKey(site),
        conflict: siteConflictPublicSummary(site),
        lastBattleHour: Number(state.worldHour || 0),
        actors
      }
  ...
  function fixedLairInCooldown(zone = null, slot = null, now = Number(state.worldHour || 0)) {
    if (!zone || !slot || zone.status !== 'looted') return false;
    const details = zone.details && typeof zone.details === 'object' ? zone.details : {};
    if (Number(details.fixedLairVersion || 0) !== FIXED_LAIR_STATE_VERSION) return false;
    const clearedHour = Number(zone.resolvedHour || details.clearedHour || 0);
    const respawnHours = Math.max(1, Number(details.fixedLairRespawnHours || slot.respawnHours || FIXED_LAIR_RESPAWN_HOURS));
    return Number(now || 0) < clearedHour + respawnHours;
  }

  function restoreFixedLairParty(party = null, slot = null) {
    if (!party || !slot) return false;
    const defaults = defaultParties()[party.id] || {};
    const group = factionGroup(defaults.faction || party.faction || '');
    party.kind = String(defaults.kind || party.kind || 'monster').slice(0, 32);
    party.faction = safeId(defaults.faction || party.faction || 'wild', 'wild');
    const kind = String(party.kind || '').toLowerCase();
    if (group === 'mutants') {
      party.species = safeId(defaults.species || 'mutant', 'mutant');
    } else if (kind === 'monster' || group === 'wild') {
      party.species = safeId(defaults.species || party.species || wildCreatureProfileForParty({ ...party, ...defaults }).visual || '', '');
    } else {
      party.species = '';
    }
    party.destroyed = false;
    party.state = String(defaults.state || (group === 'raiders' ? 'hunting' : 'roaming')).slice(0, 32);
    party.homeSiteId = safeId(defaults.homeSiteId || party.homeSiteId || slot.siteId || 'settlement', 'settlement');
    party.destinationSiteId = safeId(defaults.destinationSiteId || party.destinationSiteId || party.homeSiteId || '', '');
    ...
    party.routeIndex = 0;
    party.x = Number(slot.x || defaults.x || party.x || 0);
    party.y = Number(slot.y || defaults.y || party.y || 0);
    party.strength = clamp(defaults.strength || party.strength || 10, 1, 500);
    party.members = clamp(defaults.members || party.members || 1, 1, 200);
    party.respawnHours = clamp(defaults.respawnHours || party.respawnHours || FIXED_LAIR_RESPAWN_HOURS, 1, 240);
    party.baseSpeedKmh = clamp(defaults.baseSpeedKmh || party.baseSpeedKmh || defaults.speedKmh || party.speedKmh || 20, 1, 90);
    party.speedKmh = normalizeWorldPartySpeedKmh(party, defaults);
    party.speedProfileVersion = WORLD_PARTY_SPEED_PROFILE_VERSION;
    party.cargo = {};
    party.engagedZoneId = '';
    ...
    clearCaravanStaging(party);
    party.recoverUntilHour = 0;
    party.lastSiteId = slot.siteId || party.homeSiteId || '';
  ...
  function worldContactActionForKind(kind = '') {
    const key = String(kind || '').toLowerCase();
    if (key === 'lair') return 'Зачистить';
    if (key === 'raid' || key === 'siege' || key === 'battle') return 'Вмешаться';
    if (key === 'caravan') return 'Перехватить';
    if (key === 'aftermath') return 'Осмотреть';
    return 'Проверить';
  }

  function addWorldContact(rows, seen, contact = {}) {
    const point = normalizeWorldPoint(contact);
    if (!point) return null;
    const id = safeId(contact.id || `${contact.kind || 'contact'}_${Math.round(point.x)}_${Math.round(point.y)}`, 'contact');
    const siteId = String(contact.siteId || '').slice(0, 80);
    const partyId = String(contact.partyId || '').slice(0, 80);
    const zoneId = String(contact.zoneId || '').slice(0, 80);
    const bucket = contact.details?.partyMeeting && partyId
      ? `party:${partyId}`
      : `${Math.round(point.x / 10)}:${Math.round(point.y / 10)}`;
    const priority = clamp(contact.priority || 1, 1, 5);
    const existing = seen.get(bucket);
    if (existing && Number(existing.priority || 0) >= priority) return existing;
    const encounterId = String(contact.encounterId || encounterIdForWorldContact(contact)).slice(0, 64);
    const hidden = !!contact.hidden || contact.visible === false
      || !!contact.details?.hidden || contact.details?.visible === false;
    const row = {
      id,
      kind: String(contact.kind || 'event').slice(0, 32),
      title: String(contact.title || 'Событие пустоши').slice(0, 120),
      text: String(contact.text || '').slice(0, 360),
      status: String(contact.status || 'active').slice(0, 32),
      actionLabel: String(contact.actionLabel || worldContactActionForKind(contact.kind)).slice(0, 32),
      x: Number(point.x.toFixed(2)),
      y: Number(point.y.toFixed(2)),
      priority,
      sourceType: String(contact.sourceType || '').slice(0, 32),
      sourceId: String(contact.sourceId || '').slice(0, 80),
      zoneId,
      roomId: String(contact.roomId || '').replace(/[^a-zA-Z0-9_#-]/g, '').slice(0, 96),
      radius: clamp(contact.radius ?? 0, 0, 30),
      siteId,
      partyId,
      threatPartyId: String(contact.threatPartyId || '').slice(0, 80),
      faction: String(contact.faction || '').slice(0, 64),
      targetFaction: String(contact.targetFaction || '').slice(0, 64),
      encounterId,
      locationId: String(locationIdForWorldContact({ ...contact, encounterId })).slice(0, 80),
      pvpMode: String(contact.pvpMode || 'pvp').slice(0, 32),
      expiresHour: Math.max(0, Number(contact.expiresHour || 0)),
      worldHour: Number(Number(state.worldHour || 0).toFixed(2)),
      hidden,
      visible: !hidden,
      details: contact.details && typeof contact.details === 'object' ? clone(contact.details) : {}
    };
    if (existing) {
  ...
  function publicWorldContacts(limit = 18) {
    void limit;
    cleanupWorldZonesForSingleReality();
    return [];
    ...
    const seen = new Map();
    const now = Number(state.worldHour || 0);
    const activeTasks = state.worldTasks
      .filter(task => task && task.status === 'active')
      .sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0));

    (Array.isArray(state.worldZones) ? state.worldZones : [])
      .filter(zone => zone && zone.status === 'active')
      .filter(zone => !Number(zone.expiresHour || 0) || Number(zone.expiresHour || 0) > now)
      .forEach(zone => {
        const active = zone.status === 'active';
        const locationId = worldZoneLocationId(zone);
        addWorldContact(rows, seen, {
          id: `zone_${zone.id}`,
          zoneId: zone.id,
          roomId: worldZoneRoomId(zone),
          kind: active ? zone.kind : 'aftermath',
          status: zone.status,
          title: active ? zone.title : `Разграблено: ${zone.title}`,
          text: active
            ? zone.text
            : 'Эту точку уже зачистили. Она остаётся на карте как след разграбленного места.',
          x: zone.x,
          y: zone.y,
          radius: zone.radius,
          priority: active ? Math.max(2, Number(zone.priority || 1)) : 1,
          sourceType: zone.sourceType || 'world_zone',
          sourceId: zone.sourceId || zone.id,
          siteId: zone.siteId || '',
          partyId: zone.partyId || '',
          threatPartyId: zone.threatPartyId || '',
          faction: zone.faction || '',
          targetFaction: zone.targetFaction || '',
          encounterId: worldZoneEncounterId(zone),
          locationId,
          pvpMode: zone.pvpMode || 'pvp',
          expiresHour: zone.expiresHour || 0,
          actionLabel: active ? worldContactActionForKind(zone.kind) : 'Осмотреть',
          hidden: !!zone.details?.hidden || zone.details?.visible === false,
          visible: !(zone.details?.hidden || zone.details?.visible === false),
          details: { ...(zone.details || {}), worldZone: true, zoneStatus: zone.status }
        });
      });

    activeTasks.forEach(task => {
      const type = String(task.type || '').toLowerCase();
      if (!['defend_resource', 'retake_site', 'clear_lair'].includes(type)) return;
      const site = task.siteId ? state.sites[task.siteId] : null;
      if (site && activeSiteConflict(site) && ['defend_resource', 'retake_site'].includes(type)) return;
      const party = task.partyId ? state.parties[task.partyId] : null;
      const detailsPoint = normalizeWorldPoint(task.details || {});
      const taskZone = type === 'clear_lair'
        ? worldZoneById(task.details?.worldZoneId || task.worldZoneId || '')
        : null;
      const target = type === 'clear_lair' && detailsPoint
        ? detailsPoint
        : (party && !party.destroyed && party.state !== 'destroyed' ? party : site);
      if (!target) return;
      const threatFaction = task.targetFaction || partyThreatInfo(party || {}).threatFaction || site?.lastRaidFaction || '';
      const kind = type === 'escort_caravan'
        ? 'caravan'
        : type === 'join_patrol'
          ? 'battle'
          : type === 'retake_site'
            ...
            : type === 'clear_lair'
              ? 'lair'
              : (task.objective === 'hold_control' ? 'siege' : 'raid');
      const encounterId = type === 'clear_lair'
        ? String((taskZone ? worldZoneEncounterId(taskZone) : '') || task.details?.encounterId || (party ? partyMeetingEncounterId(party) : encounterIdForWorldContact({
          kind,
          targetFaction: threatFaction,
          partyId: task.partyId || '',
          source: type,
          name: task.details?.threatName || task.title || ''
        }))).slice(0, 64)
        : encounterIdForWorldContact({ kind, targetFaction: threatFaction, source: type });
      const locationId = type === 'clear_lair'
        ? String(taskZone ? worldZoneLocationId(taskZone) : (party ? lairLocationIdForParty(party) : (task.details?.locationId || locationIdForWorldContact({ kind, targetFaction: threatFaction, encounterId })))).slice(0, 80)
        : (type === 'escort_caravan' || type === 'join_patrol')
          ? ''
          : String(site?.locationId || locationIdForWorldContact({ kind, targetFaction: threatFaction, source: type })).slice(0, 80);
      addWorldContact(rows, seen, {
        id: `task_${task.id}`,
        kind,
        title: task.title || 'Срочное событие',
        text: task.text || '',
        x: target.x,
        y: target.y,
        priority: Math.max(2, Number(task.priority || 1)),
        sourceType: 'task',
        sourceId: task.id,
        siteId: task.siteId || site?.id || '',
        partyId: task.partyId || party?.id || '',
        faction: party?.faction || site?.owner || '',
        targetFaction: threatFaction,
        encounterId,
        locationId,
        roomId: taskZone
          ? worldZoneRoomId(taskZone)
          : type === 'clear_lair'
            ? stableWorldRoomId(locationId || locationIdForWorldContact({ kind, targetFaction: threatFaction }), `task_${task.id}`)
          : sharedRealityRoomId(locationId || locationIdForWorldContact({ kind, targetFaction: threatFaction })),
        pvpMode: site?.pvpMode || 'pvp',
        expiresHour: task.expiresHour || 0,
        details: { ...(task.details || {}), encounterId, locationId, taskId: task.id, objective: task.objective || '', reward: task.reward || {} }
      });
    });
    ...
    Object.values(state.sites).forEach(site => {
      ...
      const conflict = activeSiteConflict(site);
      if (!conflict) return;
      const zone = ensureSiteConflictZone(site);
      const primaryFaction = zone?.targetFaction || siteConflictPrimaryFaction(site);
      const siteLocationId = String(zone?.locationId || site.locationId || locationIdForWorldContact({ kind: 'raid', targetFaction: primaryFaction })).slice(0, 80);
      addWorldContact(rows, seen, {
        id: zone ? `zone_${zone.id}` : `conflict_${site.id}`,
        zoneId: zone?.id || '',
        roomId: zone ? worldZoneRoomId(zone) : sharedRealityRoomId(siteLocationId),
        kind: 'raid',
        title: zone?.title || siteConflictTitle(site),
        text: zone?.text || siteConflictText(site),
        x: site.x,
        y: site.y,
        priority: conflict && Number(conflict.progress || 0) >= 8 ? 5 : 4,
        sourceType: zone?.sourceType || 'site',
        sourceId: site.id,
        siteId: site.id,
        faction: site.owner || '',
        targetFaction: primaryFaction,
        encounterId: zone?.encounterId || encounterIdForWorldContact({ kind: 'raid', targetFaction: primaryFaction }),
        ...
        details: {
          ...(zone?.details || {}),
          controlPressure: site.controlPressure || 0,
          security: site.security || 0,
          conflict: conflict ? siteConflictPublicSummary(site) : null
        }
      });
    });
    ...
    Object.values(state.parties).forEach(party => {
      if (!party || party.destroyed || party.state === 'destroyed') return;
      if (activeBattleZoneForParty(party.id)) return;
      const threat = partyThreatInfo(party);
      if (String(party.kind || '') === 'caravan' && Number(threat.riskLevel || 0) >= CARAVAN_PUBLIC_THREAT_RISK) {
        addWorldContact(rows, seen, {
          id: `caravan_threat_${party.id}`,
          kind: 'caravan',
          title: `Караван под угрозой: ${party.name || party.id}`,
          text: `${partyPublicStatusText(party, threat)}. Игрок может перехватить маршрут; если угроза догонит караван, серверный бой начнется автоматически.`,
          x: party.x,
          y: party.y,
          priority: Number(threat.riskLevel || 0) >= 75 ? 5 : 3,
          sourceType: 'party',
          sourceId: party.id,
          partyId: party.id,
          threatPartyId: threat.threatPartyId || '',
          faction: party.faction || '',
          targetFaction: threat.threatFaction || '',
          encounterId: encounterIdForWorldContact({ kind: 'caravan', targetFaction: threat.threatFaction || '', source: 'caravan' }),
          locationId: 'randomRuinedRoad',
          roomId: stableWorldRoomId('randomRuinedRoad', `caravan_${party.id}`),
          radius: 9,
          pvpMode: 'pvp',
          details: {
            riskLevel: threat.riskLevel,
      ...
      const contact = partyMeetingContact(party, threat);
      if (contact) {
        addWorldContact(rows, seen, contact);
        return;
      }
    });

    return rows
      .sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0) || Number(a.expiresHour || 999999) - Number(b.expiresHour || 999999))
      .slice(0, limit);
  }
