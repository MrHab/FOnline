...
function createWastelandSimulation(options = {}) {
  ...
  function ensureResourceExpeditionTasks() {
    ...
    for (const site of sites.slice(0, openSlots)) {
      ...
      const task = createWorldTask('resource_expedition', {
        key: `resource_expedition:${site.id}`,
        title: `Вылазка: ${site.name}`,
        text: `Доберитесь до ${site.name}, соберите ${targetUnits} единиц местных ресурсов и эвакуируйтесь. Шум добычи привлекает угрозы; за ${bonusUnits} единиц награда выше.`,
        siteId: site.id,
        objective: 'resource_expedition',
        durationHours: 48,
        priority,
        reward: rewardForWorldTask('resource_expedition', site, priority),
        details: {
  ...
  function ensureDistressSignalTasks() {
    ...
    const openSlots = Math.max(0, Math.min(1 - active.length, MAX_WORLD_TASK_COUNT - totalActive));
    if (openSlots <= 0) return active;
    const activityTypes = new Set(['resource_expedition', 'recon_expedition', 'outpost_defense', 'distress_signal']);
    const occupiedSiteIds = new Set(state.worldTasks
      .filter(task => task?.status === 'active' && activityTypes.has(task.type))
      .map(task => String(task.siteId || ''))
      .filter(Boolean));
    const sites = Object.values(state.sites || {})
      .filter(site => site
        && siteIsInPublicRelease(site)
    ...
    for (const site of sites.slice(0, openSlots)) {
      ...
      const maxKills = 9;
      const priority = clamp(2 + Math.round(Number(site.danger || 0) / 2), 2, 4);
      const task = createWorldTask('distress_signal', {
        key: `distress_signal:${site.id}`,
        title: `Сигнал бедствия: ${site.name}`,
        text: `Найдите источник сигнала у ${site.name}, активируйте маяк и зачистите засаду. После ${targetKills} поверженных врагов помощь засчитывается; ${bonusKills} дают повышенную награду.`,
        siteId: site.id,
        objective: 'distress_signal',
        durationHours: 18,
        priority,
        reward: rewardForWorldTask('distress_signal', site, priority),
        details: {
  ...
  function completeWorldActivityTask(taskId = '', data = {}) {
    const id = String(taskId || '').trim();
    const task = state.worldTasks.find(row => row && String(row.id || '') === id);
    if (!task || task.status !== 'active') return { ok: false, error: 'Задание уже недоступно.' };
    ...
    task.reward = {
      ...
      caps: Math.round(Math.max(0, Number(task.reward?.caps || 0)) * rewardMultiplier)
    };
    fundWorldTaskCapsRewardFromSite(task, worldTaskPayerSite(site), 1);
    const rewardCharacterIds = [...new Set((Array.isArray(data.rewardCharacterIds) ? data.rewardCharacterIds : [])
    ...
    } else if (activityKind === 'distress_signal') {
  ...
  /**
   * Караванов на карте нет (экономика v3): постоянные караваны возвращаются при
   * каждой загрузке из defaultParties и старых сохранений, поэтому их убирают
   * на каждом шаге. Груз в пути закрывается в журнале, привязанные задания
   * сопровождения истекают обычным путём, раз партии больше нет.
   */
  function retireWorldCaravans() {
    ...
    if (Object.keys(state.refugeeFlows?.active || {}).length) {
      // Без населения нет и беженцев: группы из старых сохранений просто расходятся.
  ...
  /**
   * Враждебных отрядов на карте нет: угрозы пустоши — группы A-Life опасных
   * клеток. Постоянные налётчики, мутанты и звери возвращаются из defaultParties
   * при каждой загрузке, поэтому их убирают на каждом шаге вместе с их встречами
   * и боями.
   */
  ...
  function publicThreatZones() {
    // Отряды NPC скрыты и не выдают себя кругами угрозы: круги есть только у мест.
    const zones = [];
