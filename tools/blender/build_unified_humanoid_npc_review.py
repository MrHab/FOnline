...
# Клипы локомоции для движения назад и в приседе строит Node-запекатель
# tools/bake-authored-locomotion-clips.js: он зеркалит траекторию лодыжки
# источника и досчитывает ноги IK. Blender отвечает только за базу.
...
BAKED_AFTERWARDS = tuple(LOCOMOTION_CLIP_TABLES["clips"].keys())
...
def main() -> None:
    ...
    report = {
        ...
        "bakedAfterwards": list(BAKED_AFTERWARDS),
        "actualGlb": actual,
        "provenance": {
