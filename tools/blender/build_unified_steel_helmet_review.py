...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--asset-id", default="equipment_steel_helmet_unified_v1")
...
def build_helmet_mesh(
    ...
    # Stamped crown ridge follows the front/back profile and gives the helmet a
    # specific, readable identity even at the game camera distance.
    ridge_points: list[Vector] = []
    for index in range(9):
        y_factor = -0.82 + 1.64 * index / 8
        progression = abs(y_factor) * 0.74
        angle = -pi * 0.5 if y_factor < 0 else pi * 0.5
        point = dome_point(dimensions, angle, progression)
        point.z += 0.008
        ridge_points.append(point)
    for start, end in zip(ridge_points, ridge_points[1:]):
        builder.box_between(start, end, Vector((1, 0, 0)), 0.008, 0.004, 1)

    # Four retaining rivets.
    for side in (-1.0, 1.0):
        for back_sign in (-1.0, 1.0):
            angle = (0.18 if back_sign > 0 else -0.18) + (0.0 if side > 0 else pi)
            point = dome_point(dimensions, angle, 0.89)
            outward = Vector((side, back_sign * 0.22, 0.10)).normalized()
            builder.surface_disc(point + outward * 0.002, outward, 0.0065, 1, 8)

    # Chin straps: broad flat leather, with the buckle on the character's left.
    strap_front_y = dimensions["centerY"] - dimensions["frontRadius"] * 0.66
    for side in (-1.0, 1.0):
        start = Vector((dimensions["centerX"] + side * (dimensions["radiusX"] - 0.005), dimensions["centerY"] - 0.006, dimensions["rimZ"] - 0.016))
        middle = Vector((dimensions["centerX"] + side * 0.070, strap_front_y, dimensions["rimZ"] - 0.068))
        end = Vector((dimensions["centerX"] + side * 0.030, strap_front_y - 0.012, dimensions["rimZ"] - 0.118))
        builder.box_between(start, middle, Vector((0, 1, 0)), 0.018, 0.005, 3)
        builder.box_between(middle, end, Vector((0, 1, 0)), 0.018, 0.005, 3)
    buckle_center = Vector((dimensions["centerX"] - 0.056, strap_front_y - 0.004, dimensions["rimZ"] - 0.082))
    builder.box_between(buckle_center + Vector((-0.014, 0, 0)), buckle_center + Vector((0.014, 0, 0)), Vector((0, 0, 1)), 0.010, 0.006, 1)
    builder.box_between(buckle_center + Vector((0, 0, -0.014)), buckle_center + Vector((0, 0, 0.014)), Vector((1, 0, 0)), 0.010, 0.006, 1)

    # Sparse rust scars and a welded repair tab.  These are intentional focal
    # points; the texture supplies the finer all-over wear.
    for angle, progression, radius in ((-2.10, 0.72, 0.0085), (0.82, 0.64, 0.006), (-0.48, 0.46, 0.005)):
        point = dome_point(dimensions, angle, progression)
...
def main() -> None:
    ...
    materials = [
        pbr_material("helmet_weathered_steel", (0.115, 0.132, 0.125), 0.61, 0.78, args.body_id, 0.18),
        pbr_material("helmet_worn_edges", (0.255, 0.285, 0.275), 0.43, 0.86, args.body_id, 0.10),
        pbr_material("helmet_oxidized_scars", (0.155, 0.042, 0.014), 0.91, 0.10, args.body_id, 0.20),
        pbr_material("helmet_dark_leather_strap", (0.060, 0.027, 0.014), 0.83, 0.0, args.body_id, 0.14),
    ]
    ...
    print("REALM_UNIFIED_STEEL_HELMET_V1=" + json.dumps(report, ensure_ascii=False, sort_keys=True))
