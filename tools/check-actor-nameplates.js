...
// Без подложки читаемость держится на контуре: обводка даёт ровный край,
// тени — плотность. Одних полупрозрачных теней на светлом песке не хватало.
assert(/-webkit-text-stroke:\s*[\d.]+px\s*#000/.test(plateRule[1]),
  'у имён и здоровья нет чёрного контура — на светлом песке они сольются с фоном');
assert(/text-shadow:[^;]*#000/.test(plateRule[1]),
  'контур имён и здоровья держится только на обводке и поплывёт на пёстром фоне');
