  ...
  objects: [
    {
      ...
      rotation: { y: 1.5 },
      activitySlots: [
        { id: 'named_post', type: 'guard', capacity: 1, ownerNpcId: 'named', visualAction: 'guard' },
        { id: 'shared_post', type: 'guard', capacity: 1, position: { x: 6, y: 0, z: 8 } }
      ]
    },
    {
      id: 'fire',
...
assert.deepStrictEqual(slots.find(slot => slot.id === 'named_post').position, { x: 4, y: 0, z: 8 });
assert.strictEqual(slots.find(slot => slot.id === 'named_post').facing, 1.5);
...
const namedSlot = reserveActivitySlot({ slots, reservations, npc: named, target: { slotId: 'named_post' }, locationId: location.id });
assert.strictEqual(namedSlot?.id, 'named_post', 'owner should reserve their personal slot');
assert.strictEqual(
  reserveActivitySlot({ slots, reservations, npc: workerA, target: { slotId: 'named_post' }, locationId: location.id }),
  null,
...
assert.strictEqual(workerSlot?.id, 'shared_post');
assert.strictEqual(
...
const fastReservations = new Map([['shared_post', new Set([workerB.id])]]);
