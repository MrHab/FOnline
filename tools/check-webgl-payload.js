...
// весит в сборке 37 МБ. Поэтому в каталоге Resources держатся только тела
// персонажа и библиотека анимаций, твари грузятся по URL, а текстурам
...
assert.equal(urls.length, 7, `в каталоге Resources должно быть 7 записей, найдено ${urls.length}`);
...
assert(/ExpectedRuntimeCount\s*=\s*7\b/.test(probe), 'проба каталога должна ожидать 7 записей');
